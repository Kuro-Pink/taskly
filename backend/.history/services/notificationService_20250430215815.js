...
export const sendNotificationToProject  = (projectId, event, data) => {
  console.log("projectId", projectId, event, data);
  
  if (io) {
    io.to(projectId).emit(event, data); //logic chưa hiểu, ý là đây là thông báo chỉ gửi tới một userId nào đó như comment trong issue của bạn
                                     //hoặc là khi được giao việc, trả lời bình luận chẳng hạn
  }
};
