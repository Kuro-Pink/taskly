...
import { sendNotificationToProject } from '../services/notificationService.js'; // Đảm bảo bạn có file này để xử lý WebSocket.
...
export const createIssue = async (req, res) => {
    try {
        ...
        console.log(existingProject._id);
       ...
       // Gửi thông báo qua WebSocket (có thể sử dụng socket để phát sự kiện cho các thành viên dự án)
        await sendNotificationToProject(SOCKET_EVENTS.RECEIVE_ACTIVITY, {
            _id: "",
...
export const updateIssue = async (req, res) => {
    try {
        ...
        await sendNotificationToProject(SOCKET_EVENTS.RECEIVE_ACTIVITY, {
            ...
            target: `Issue ${existingProject.key} - ${updatedIssue.number}`,    // "issue #123", "task #789"
            timestamp: Date.now()
            }
        );
...
export const deleteIssue = async (req, res) => {
    try {
        ...
        sendNotificationToProject(SOCKET_EVENTS.RECEIVE_ACTIVITY, {
            _id: "",
            username: user.name,
            action: "đã xóa",
