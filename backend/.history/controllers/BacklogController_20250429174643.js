...
import { sendWebSocketUpdate } ../services/notificationService.js'; // Đảm bảo bạn có file này để xử lý WebSocket.
...
export const createIssue = async (req, res) => {
    try {
       ...
       // Gửi thông báo qua WebSocket (có thể sử dụng socket để phát sự kiện cho các thành viên dự án)
        await sendWebSocketUpdate(SOCKET_EVENTS.RECEIVE_ACTIVITY, {
            _id: "",
...
export const updateIssue = async (req, res) => {
    try {
        ...
        await sendWebSocketUpdate(SOCKET_EVENTS.RECEIVE_ACTIVITY, {
            ...
            target: `Issue ${existingProject.key} - ${updatedIssue.title}`,    // "issue #123", "task #789"
            timestamp: Date.now()
            }
        );
...
export const deleteIssue = async (req, res) => {
    try {
        ...
        sendWebSocketUpdate(SOCKET_EVENTS.RECEIVE_ACTIVITY, {
            _id: "",
            username: user.name,
            action: "đã xóa",
            target: `Issue ${existingProject.key} - ${issue.title}`,
            timestamp: Date.now()
        });
