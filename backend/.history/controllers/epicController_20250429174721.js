...
import { rom '../services/notificationService.js'; // Đảm bảo bạn có file này để xử lý WebSocket.
  ...
  try {
    ...
    // Gửi thông báo qua WebSocket
     ...
     sendWebSocketUpdate(SOCKET_EVENTS.RECEIVE_ACTIVITY, {
      ...
      target: `Epic ${updatedEpic.name}`,    // "issue #123", "task #789"
      timestamp: Date.now()
      }
  );
  } catch (err) {
...
export const deleteEpic = async (req, res) => {
  try {
    ...
    sendWebSocketUpdate(SOCKET_EVENTS.RECEIVE_ACTIVITY, {
      ...
      target: `Epic ${deletedEpic.name}`,    // "issue #123", "task #789"
      timestamp: Date.now()
      }
  );
