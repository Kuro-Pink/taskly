...
export const getCommentsByIssue = async (req, res) => {
  try {
    ...
    const comments = await Comment.find({ issue: issueId })
      .populate('user', 'name avatar') // ⚠️ Đảm bảo là 'user' nếu schema dùng 'user'
      .sort({ createdAt: 1 });
...
export const createComment = async (req, res) => {
  try {
    const { issue, content, userId } = req.body;
    // const userId = req.user?.id;
    console.log(issue, content, userId);
    
    if (!issue || !content || !userId) {
