...
export const removeMember = async (req, res) => {
  try {
    ...
    // Kiểm tra quyền của chủ dự án hoặc Lead
    const owner = project.members.find(m => m.user.toString() === req.user.id && m.role === "Owner");
    if (!owner) return res.status(403).json({ message: "Bạn không có quyền xóa thành viên" });

    // Xóa thành viên khỏi dự án
    const updatedMembers = project.members.filter(m => m.user.toString() !== userId);
    
    // Kiểm tra nếu không có thành viên nào thay đổi
    if (updatedMembers.length === project.members.length) {
      return res.status(400).json({ message: "Thành viên không tồn tại trong dự án" });
    }
