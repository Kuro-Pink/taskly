...
export const createComment = async (req, res) => {
  const { issueId, content, userId } = req.body;
   ...
   const savedComment = new Comment({ issue: issueId,
    content,
    user: userId, });

  // Gửi thông báo realtime cho tất cả người dùng theo dõi issue
  sendWebSocketUpdateToIssue(issueId, SOCKET_EVENTS.CREATE_COMMENT, {
    comment: newComment,
    issueId,
    receivers: [userId]  // Người dùng đã bình luận hoặc người khác cũng có thể nhận thông báo
});
