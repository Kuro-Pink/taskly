...
export const requestJoinProject = async (req, res) => {
  try {
    ...
    sendNotificationToUser(project.owner, `Yêu cầu tham gia dự án từ ${req.user.name}`);
...
export const respondJoinRequest = async (req, res) => {
  try {
    ...
    if (action === "accept") {
      ...
      sendNotificationToUser(userId, "Bạn đã được chấp nhận vào dự án!");

      // Gửi activity log cho toàn bộ project
      sendWebSocketUpdate('activity', {
