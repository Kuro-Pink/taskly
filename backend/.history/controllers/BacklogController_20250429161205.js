...
export const createIssue = async (req, res) => {
    try {
       ...
       sendNotificationToUser(userId, SOCKET_EVENTS.RECEIVE_ACTIVITY, {
            message: `🆕 Issue ${newIssue.title} đã được tạo`,
            username: user.name,       // (tuỳ loại)
            action: "đã tạo",    // "updated", "commented", "assigned"
            target: `Issue ${existingProject.key} - ${newIssue.title}`,    // "issue #123", "task #789"
            timestamp: Date.now()
            }
        );
...
export const updateIssue = async (req, res) => {
    try {
        ...
        sendNotificationToUser(userId, SOCKET_EVENTS.RECEIVE_ACTIVITY, {
            message: `🆕 Issue ${newIssue.title} đã được tạo`,
            username: user.name,       // (tuỳ loại)
            action: "đã tạo issue",    // "updated", "commented", "assigned"
            target: newIssue.title,    // "issue #123", "task #789"
            timestamp: Date.now()
            }
        );
...
export const deleteIssue = async (req, res) => {
    try {
        ...
        // Gửi thông báo qua WebSocket
         sendNotificationToUser(userId,"DELETE_ISSUE", deletedIssue);
...
export const createSprint = async (req, res) => {
    try {
        ...
        // Gửi thông báo qua WebSocket
       sendNotificationToUser(userId,"CREATE_SPRINT", newSprint);
...
export const updateSprint = async (req, res) => {
    try {
        ...
        res.json(updatedSprint);
         // Gửi thông báo qua WebSocket
        sendNotificationToUser(userId,"UPDATE_SPRINT",updatedSprint);
    } catch (error) {
...
export const deleteSprint = async (req, res) => {
    try {
        ...
        res.json({ message: 'Xóa sprint thành công và cập nhật các issue về backlog' });
         ...
         // Gửi thông báo qua WebSocket
         sendNotificationToUser(userId,"DELETE_SPRINT", deletedSprint);
    } catch (error) {
