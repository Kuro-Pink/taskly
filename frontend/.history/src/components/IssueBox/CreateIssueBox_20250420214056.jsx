...
const CreateIssueBox = ({ sprintId = null }) => {
    const dispatch = useDispatch();
    const { key, id } = useParams(); // 🟢 Lấy giá trị từ URL

    const state = useSelector((state) => state);
    console.log('Global Redux State:', state);
    ...
    const handleAddIssue = () => {
        if (!newIssue.title.trim() || !currentProject || !currentProject._id) return;
        console.log(currentProject?._id, 'currentProject?._id');
