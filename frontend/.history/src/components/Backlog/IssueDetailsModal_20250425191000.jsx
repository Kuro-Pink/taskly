...
import { getCommentsByIssue, createComment, updateComment, deleteComment } from '../../redux/featuresres/commentSlice'; // sửa đường dẫn nếu cần
...
const IssueDetailsModal = ({ issue, onClose }) => {
    ...
    useEffect(() => {
        if (issue) {
            dispatch(getCommentsByIssue(issue));
        }
    }, [issue]);
                ...
                <div className="h-[612px] overflow-y-auto px-8 py-6">
                    ...
                    <div>
                        ...
                        <button
                            onClick={() => {
                                ...
                                dispatch(createComment({ issueId: currentIssueId, content: newComment.trim() }));
                                setNewComment('');
                            }}
                                ...
                                <div key={cmt._id} className="flex items-start gap-3">
                                    ...
                                    <div className="flex-1">
                                        <div className="flex justify-between text-base text-gray-600">
                                            <span className="font-semibold">{cmt.user?.name || 'Unknown'}</span>
                                        ...
                                        {user?._id === cmt.user?._id && (
                                            <div className="mt-1 space-x-2 text-sm text-blue-600">
                                                <button onClick={() => handleEditComment(cmt)}>Edit</button>
                                                <button onClick={() => dispatch(deleteComment(cmt._id))}>Delete</button>
                                            </div>
                                        )}
                                    </div>
                                </div>
                    ...
                    {editCommentId === cmt._id ? (
                        <div>
                            ...
                            />
                            <button
                                className="mr-2 rounded bg-green-600 px-2 py-1 text-sm text-white"
                                onClick={() => {
                                    dispatch(updateComment({ commentId: cmt._id, content: editContent }));
                                    setEditCommentId(null);
                                }}
                            >
                                Save
                            </button>
                            <button className="text-sm text-gray-600" onClick={() => setEditCommentId(null)}>
                                Cancel
                            </button>
                        </div>
                    ) : (
