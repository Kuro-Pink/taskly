...
import { updateIssue, fetchAllIssue } from '../../redux/features/backlogSlice';
...
const IssueDetailsModal = ({ issue, onClose, onUpdate }) => {
    if (!issue) return null;

    const [editingIssueId, setEditingIssueId] = useState(issue._id || null);
    ...
    const handleSaveEdit = (issueId) => {
        ...
        dispatch(updateIssue({ issueId, updates: { title: editedTitle } }))
            .unwrap()
            .then(() => {
                dispatch(fetchAllIssue()).then((res) => {
                    // Cập nhật lại selectedIssue sau khi fetchAll
                    const updated = res.payload.find((i) => i._id === issue._id);
