                ...
                <div className="h-[612px] overflow-y-auto px-8 py-6">
                    ...
                    <div className="mb-8 grid grid-cols-2 gap-6">
                        ...
                        <div className="mb-4">
                            ...
                            {/* Hiển thị tên Epic hiện tại */}
                            {issue?.epic ? (
                                <h2 className="mb-2 rounded border bg-gray-100 px-3 py-1 text-sm text-gray-800">
                                    {epics.find((epic) => epic._id === issue.epic)?.name || 'Không tìm thấy'}
                                </h2>
                            ) : (
                                <h2 className="mb-2 rounded border px-3 py-1 text-sm text-gray-400 italic">
                                    Chưa có Epic
                                </h2>
                            )}

                            {/* Dropdown select Epic */}
                            <select
                                value={issue.epic || ''}
                                onChange={(e) => {
                                    const selectedEpic = e.target.value || null;
                                    dispatch(
                                        updateIssue({
                                            issueId: issue._id,
                                            updates: { epic: selectedEpic },
                                        }),
                                    )
                                        .unwrap()
                                        .then(() => dispatch(fetchIssueById(issue._id)))
                                        .catch(console.error);
                                }}
                                className="w-full rounded border px-3 py-2 text-sm text-gray-700 shadow-sm focus:border-blue-500 focus:ring focus:outline-none"
                            >
