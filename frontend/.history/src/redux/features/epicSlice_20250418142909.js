...
export const fetchEpicsByProject = createAsyncThunk('epics/fetchByProject', async (projectId, thunkAPI) => {
    try {
...
const epicSlice = createSlice({
    ...
    extraReducers: (builder) => {
        builder
            ...
            // FETCH
            .addCase(fetchEpicsByProject.pending, (state) => {
                state.loading = true;
                state.error = null;
            })
            .addCase(fetchEpicsByProject.fulfilled, (state, action) => {
                state.loading = false;
                state.items = action.payload;
            })
            .addCase(fetchEpicsByProject.rejected, (state, action) => {
                state.loading = false;
