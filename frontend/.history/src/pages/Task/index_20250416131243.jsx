...
const KanbanBoard = () => {
    ...
    const startedSprintIds = sprints?.filter(
        (sprint) => sprint.started && String(sprint.project) === String(currentProject?._id),
    );

    const filteredIssues = issues?.filter((issue) => {
        const isInCurrentProject = String(issue.project) === String(currentProject?._id);
        const isInStartedSprint = startedSprintIds.find((s) => s._id === issue.sprint);
        const hasStatusOnly = !issue.sprint && issue.status; // Không thuộc sprint nhưng đã gán status

        return isInCurrentProject && (isInStartedSprint || hasStatusOnly);
    });
