...
const KanbanBoard = () => {
    ...
    // 1. Lấy ra danh sách sprintId đã start trong project
    const startedSprintIds = sprints
        ?.filter((s) => s.started && String(s.project) === String(currentProject?._id))
        .map((s) => String(s._id));

    // 2. Lọc issue
    const projectStatuses = statuses?.filter((st) => String(st.project) === String(currentProject?._id));

    const filteredIssues = issues?.filter((issue) => {
        // a) issue phải cùng project
        if (String(issue.project) !== String(currentProject?._id)) return false;

        // b1) issue nằm trong sprint đã start?
        const inStartedSprint = issue.sprint && startedSprintIds.includes(String(issue.sprint));

        // b2) issue có status mà status đó thuộc project?
        const hasValidStatus = projectStatuses.some((st) => String(st._id) === String(issue.status));

        return inStartedSprint || hasValidStatus;
    });

    // 3. Nhóm theo status
    const issuesByStatus = {};
    projectStatuses.forEach((st) => {
        issuesByStatus[st._id] = filteredIssues.filter((issue) => String(issue.status) === String(st._id));
    });
