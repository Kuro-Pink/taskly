...
const BacklogBoard = () => {
    ...
    const [selectedIssue, setSelectedIssue] = useSelector((state) => state.backlog.selectedIssue || null); // Lấy issue từ Redux store
    console.log('selectedIssue', selectedIssue);

    const handleIssueClick = (item) => {
        dispatch(fetchIssueById(item._id)); // Dispatch action chọn issue
    };

    useEffect(() => {
        // Giả sử issueId đã có sẵn hoặc lấy từ đâu đó
        if (!selectedIssue) return;
        dispatch(fetchIssueById(selectedIssue)); // Lấy dữ liệu issue theo id
    }, [dispatch]);
                                                ...
                                                <Draggable key={item._id} draggableId={item._id} index={index}>
                                                    {(provided) => (
                                                        <li
                                                            ...
                                                            onClick={() => handleIssueClick(item)} // Gọi dispatch khi click vào issue
                                                            // onClick={(e) => {
