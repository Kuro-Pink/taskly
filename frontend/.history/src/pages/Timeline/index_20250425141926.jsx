...
import { fetchAllIssue, fetchAllSprint, createIssue, moveIssueToEpic } from '../../redux/features/backlogSlice';
...
const Timeline = () => {
    ...
    const handleDragEnd = (result) => {
        ...
        dispatch(moveIssueToEpic({ issueId: draggableId, epicId: destId }))
            .unwrap()
            .then(() => {
                dispatch(fetchAllIssue());
            })
            .catch((error) => {
                console.error('Lỗi khi cập nhật epic của issue:', error);
            });

        setExpandedEpicIds((prev) => [...new Set([...prev, epic._id])]);
    };
                                                        ...
                                                        <div className="flex items-center gap-2">
                                                            <input
                                                                ...
                                                                onKeyDown={async (e) => {
                                                                    if (e.key === 'Enter') {
                                                                        e.preventDefault();
                                                                        await handleSaveEpicName(epic._id);
                                                                    }
                                                                }}
                                                                autoFocus
                                                            />
