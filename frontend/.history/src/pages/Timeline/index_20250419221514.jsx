...
import { fetchAllSprint, updateIssue } from '../../redux/features/backlogSlice';
...
const Timeline = () => {
    ...
    const toggleEpicExpansion = (epicId) => {
        ...
        if (updated.has(epicId)) updated.delete(epicId);
        else updated.add(epicId);
    ...
    const handleDragEnd = (result) => {
        ...
        if (sourceEpicId === destEpicId) {
            dispatch(updateIssue({ issueId, updates: { order: destination.index } }));
        } else {
            dispatch(
                updateIssue({
                    issueId,
                    updates: {
                        epic: destEpicId === 'null' ? null : destEpicId,
    ...
    const renderEpic = (epic) => {
        ...
        const issuesToShow = isExpanded ? epicIssues : epicIssues.slice(0, 3);

        return (
            <Droppable droppableId={epic._id} key={epic._id}>
                {(provided) => (
                    <div className="epic-card" ref={provided.innerRef} {...provided.droppableProps}>
                        <div className="epic-header flex items-center justify-between">
                            ...
                            <div className="epic-actions flex gap-2">
                                <button
                                    onClick={() => {
                        ...
                        <div className="epic-issues mt-2 space-y-2">
                                ...
                                <Draggable draggableId={issue._id} index={index} key={issue._id}>
                                    {(provided) => (
                                        <div
                                            ...
                                            className="rounded bg-gray-100 p-2 text-sm shadow-sm"
                                        >
                                            {issue.summary}
                                        </div>
                                    )}
                            ...
                            {epicIssues.length > 3 && (
                                <button
                                    onClick={() => toggleEpicExpansion(epic._id)}
                                    className="mt-1 text-sm text-blue-500 hover:underline"
                                >
                                    {isExpanded ? 'Ẩn bớt' : `Hiển thị tất cả (${epicIssues.length})`}
                                </button>
                            )}
                        </div>
                    </div>
    ...
    return (
        <div className="min-h-screen bg-gray-100 p-6">
            ...
            <div
                ref={ganttRef}
                className="relative max-h-[400px] overflow-x-auto rounded-lg bg-white p-4 shadow-md"
            ></div>
