...
const Timeline = () => {
    ...
    const issueTypes = [
        { name: 'Task', color: 'text-green-600' },
        { name: 'Bug', color: 'text-red-600' },
        { name: 'Story', color: 'text-blue-600' },
    ];
    export default function IssueFormInEpic({
        epic,
        addingIssueEpicId,
        newIssue,
        setNewIssue,
        handleAddIssueToEpic,
        setAddingIssueEpicId,
      }) {
        const selectedType = issueTypes.find((t) => t.name === newIssue.type)
    ...
    return (
        ...
        <div className="mt-2 mr-4 ml-6 flex items-center gap-2 rounded-md bg-white p-2 shadow">
          <Listbox
            value={newIssue.type}
            onChange={(val) => setNewIssue({ ...newIssue, type: val })}
          >
            <div className="relative w-1/4 text-sm">
              <Listbox.Button
                className={`w-full rounded-md border p-1 text-left font-semibold ${selectedType?.color}`}
              >
                {newIssue.type}
              </Listbox.Button>
              <Listbox.Options className="absolute z-10 mt-1 w-full rounded-md border bg-white shadow">
                {issueTypes.map((type) => (
                  <Listbox.Option key={type.name} value={type.name} as={Fragment}>
                    {({ active }) => (
                      <li
                        className={`cursor-pointer px-2 py-1 text-sm ${type.color} ${
                          active ? 'bg-gray-100' : ''
                        }`}
                      >
                        {type.name}
