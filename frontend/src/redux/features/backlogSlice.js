...
export const createIssue = createAsyncThunk(
    'backlog/createIssue',
    async ({ title, type, sprint, project, status, epic }, { rejectWithValue, getState }) => {
        try {
            const token = getState().auth.token;
            if (!token) return rejectWithValue({ message: 'Không có token xác thực' });
            ...
            const response = await axios.post(
                `${API_URL}/issues`,
                { title, type, sprint, project, status, epic },
                config,
            );
            return response.data;
        } catch (error) {
...
// 🟢 **Di chuyển Issue sang Epic**
export const moveIssueToEpic = createAsyncThunk(
    'backlog/moveToEpic',
    async ({ issueId, epicId }, { rejectWithValue, getState }) => {
        try {
            const token = getState().auth.token;
            if (!token) return rejectWithValue({ message: 'Không có token xác thực' });

            const config = { headers: { Authorization: `Bearer ${token}` } };
            const response = await axios.put(`${API_URL}/moveIssue/${issueId}/toEpic/${epicId}`, {}, config);
            return response.data;
        } catch (error) {
    ...
    extraReducers: (builder) => {
        builder
            ...
            .addCase(updateIssue.fulfilled, (state, action) => {
                ...
                const index = state.issues.findIndex((issue) => issue._id === updated._id || issue.id === updated._id);
                if (index !== -1) {
                    state.issues[index] = {
                        ...state.issues[index],
                        ...updated,
