...
export const requestJoinProject = createAsyncThunk(
    ...
    async ({ code }, { rejectWithValue, getState }) => {
        console.log('Requesting to join project with code:', code);
...
export const updateMemberRole = createAsyncThunk(
    ...
    async ({ projectId, userId, newRole }, { rejectWithValue, getState }) => {
        try {
            ...
            const response = await axios.put(`${API_URL}/${projectId}/members`, { userId, newRole }, config);
            return response.data;
        } catch (error) {
...
export const removeMember = createAsyncThunk(
    ...
    async ({ projectId, userId }, { rejectWithValue, getState }) => {
        try {
            ...
            const response = await axios.post(`${API_URL}/${projectId}/members`, { userId }, config);
            return response.data;
        } catch (error) {
    ...
    extraReducers: (builder) => {
        builder
            ...
            .addCase(createProject.rejected, (state, action) => {
                state.loading = false;
                state.error = action.payload?.message || 'Tạo dự án thất bại!';
            })
            // 🔵 Delete project
